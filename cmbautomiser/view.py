...
class ScheduleViewGeneric:
    ...
    def copy_selection(self):
        ...
        selection = self.tree.get_selection()
        test_string = str(self.columntypes)
        if selection.count_selected_rows() != 0:  # if selection exists
            [model, paths] = selection.get_selected_rows()
            items = []
            for path in paths:
            ...
            text = pickle.dumps([test_string, items])  # dump item as text
    ...
    def paste_at_selection(self):
        ...
        if text is not None:
            try:
                ...
                if isinstance(itemlist[1][0], data.ScheduleItemGeneric) and itemlist[0] == test_string:
                    selection = self.tree.get_selection()
                    if selection.count_selected_rows() != 0:  # if selection exists
                        [model, paths] = selection.get_selected_rows()
                        rows = []
    ...
    def set_model(self, schedule):
        """Set data model"""
        self.schedule.set_model(schedule.get_model())
        self.update_store()
    ...
    def __init__(self, parent, tree, captions, columntypes, render_funcs):
        ...
        self.schedule = data.ScheduleGeneric()
